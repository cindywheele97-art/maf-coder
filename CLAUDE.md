...
Real-mode mission bootstrap is now wired: `MissionDriver.start()` seeds one `Role.ORCHESTRATOR` task (`_orchestrator_bootstrap_task()`), so `maf-coder mission new --no-dry-run` actually runs the Orchestrator → plans → dispatches the DAG (previously a no-op). The remaining work is the **live shakedown run** itself (real Rust repo + API keys) — see `docs/FIRST_RUN_RUNBOOK.md`. `coder_provider_in_use` is now derived from the router's `coder_worker` primary (with a `maf-coder mission new --coder-provider` override), so both halves of the 异-provider rule engage on a real run. Open follow-ups noted there: Docker sandbox not yet CLI-wired, and single Orchestrator turn (no milestone re-invocation yet).
