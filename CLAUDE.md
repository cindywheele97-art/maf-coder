...
- **Egress SSRF — resolved-host check (M2):** `permissions.check_resolved_host_safe` resolves a host and rejects if **any** resolved IP is private/loopback/link-local/metadata (`ipaddress`-based `_ip_is_blocked`, covers IPv6 + IPv4-mapped). Applied to the initial URL and every redirect hop; `resolver` is injectable (real DNS in prod). Residual: resolve-then-check is still TOCTOU-vulnerable to active DNS rebinding (documented in the docstring; full fix needs pin-and-connect).
